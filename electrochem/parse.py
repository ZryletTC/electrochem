...
class EchemData:
    def __init__(
        self, dischargeCapacity, chargeCapacity, power, avgVoltage, efficiency
    ):
...
def toDataframe(path, active_mass):
    try:
        ...
        echem_df["Charge_Capacity"] = (
            echem_df["Charge_Capacity"] * 1000 * 1000 / active_mass
        )
        echem_df["Discharge_Capacity"] = (
            echem_df["Discharge_Capacity"] * 1000 * 1000 / active_mass
        )
        # Parse data
...
def extractCycleEchem(indexedData, cycleIndex):
    chargeCapacities = indexedData[cycleIndex - 1]["charge"][
        "Charge_Capacity"
    ].to_numpy()
    dischargeCapacities = indexedData[cycleIndex - 1]["discharge"][
        "Discharge_Capacity"
    ].to_numpy()
...
def extractEchem(indexedData):
    ...
    avgData = EchemData(
        np.mean(avgDischargeCap),
        np.mean(avgChargeCap),
        np.mean(avgPower),
        np.mean(avgVoltage),
        avgEfficiency,
    )
    if __name__ == "__main__":
        ...
        print(
            "Discharge Capacity [mAh/g]: %s" % (safeRound(avgData.dischargeCapacity, 2))
        )
    ...
    for index in cycleList:
        if 0 <= index < len(indexedData):
            if option == 0:
                ...
                x2_data = (
                    2 - cap1_data.iloc[-1] / full_capacity + cap2_data / full_capacity
                )
...
def generateEchemSummary(system, tablePath, indexedData, suppData):
    ...
    data["First Discharge Cap. [mAh/g]"] = safeRound(
        cycleEchemData.dischargeCapacity, acc
    )
    ...
    data["First Discharge Efficiency"] = (
        str(safeRound(cycleEchemData.efficiency, acc)) + "%"
    )
