...
# TODO: Move all this html to html files
def initDisplay(self):
    ...
    <div style="color: #202020;">
        <h2 style="text-align: center">
            Welcome to Electrochemical Parser v{self.version}!
        </h2>
        ...
        <p style="text-align:left;font-size:10pt;">
            This program tool allows researchers to quickly extract elecrochemical data
            and plot charge/discharge curves from raw excel data files generated by the
            Arbin MITS Pro electrochemical cycler. It is meant to speed up the data
            collection and analysis process for battery scientists. The tool is easy to
            use and will generate both a customizable charge/ discharge plot and a
            summary table containing information such as average voltage, capacities,
            Coulombic efficiencies, and more. For more detailed instructions and to
            access the open-source code, please refer to the project\'s github page:
...
def getVersionText(self):
    ...
    if not debug:
        ...
        if json_response:
            ...
            bodyText = re.sub(
                r"(Changelog:)", f"{newestVersion} Changelog:", bodyText
            )
    ...
    if not connected:
        versionText = f"""
        <p style="color: {color}; font-size:10pt; text-align:center">
            <br>
            ...
            visit
            <a style="color: "
             href="https://github.com/vince-wu/electrochemical-parsing/releases/latest">
                this link
            </a>
            .
            <span style="color: {boldcolor}">
        ...
        <p style="color: {color}; font-size:9pt">
            {bodyText}
        </p>
        """
...
def getJson():
    try:
        response = requests.get(
            "https://api.github.com/repos/vince-wu/electrochemical-parsing/"
            "releases/latest"
        )
...
def readInputFromSave(self):
    if os.path.isfile("savefile.txt"):
        with open("savefile.txt", "r", encoding="latin-1") as f:
            lines = f.readlines()
        for line in lines:
