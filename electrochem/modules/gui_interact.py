...
from electrochem.parse import (
    extractEchem,
    generateEchemSummary,
    parseArbin,
    toDataframe,
)
...
class InputData:
    def __init__(
        ...
        if system:
            ...
            self.system = (
                os.path.basename(dataPath).replace(".xls", "").replace("VW-", "")
            )
...
def getRawDataPath(self):
    path = QFileDialog.getOpenFileName(
        self, "Choose Raw Data File", "", "(*.xls *.csv *.res)"
    )[0]
    if path:
...
def plotData(
    self, partitioned_data, figurePath, system, initCycle, numCycles, show=True
):
...
def runTasks(self):
    try:
        ...
        generateEchemSummary(
            inputs.system,
            inputs.tablePath,
            extractedData.firstCycleData,
            extractedData.averageData,
            extractedData.voltageRange,
            inputs.mass,
            inputs.ratio,
            inputs.rate,
            inputs.type,
